 ...
 * @Description 8909审核在线申请订单
 * @Author chenmaolin
 * @Date 2019/6/28 20:06
 * Version 1.0
 **/
public class OnlineAuditOnlineapplyService implements IReceiveService, BaseUploadService {

    Logger logger = LoggerFactory.getLogger(OnlineAuditOnlineapplyService.class);
    GenerateUserIdService generateUserIdService = new GenerateUserIdService();

    private final String serverName = "[8909审核在线申请订单]";
...
//        dataMap.put("expiresIn", tokenMap.get("expiresIn"));
    ...
    private Record onlineApply2OnlineOrder(Record appMap, String userId) {
        ...
        // 订单处理类型
        record.set("processType", OrderProcessTypeEnum.ORDER_HANDSET.getValue());
    ...
    private List<Record> query() {
        Kv kv = new Kv().set(CommonAttribute.QUERY_LIMIT, SysConfig.getQueryLimit());
        SqlPara sqlPara = Db.getSqlPara("mysql.queryOnlineapplyList", kv);
