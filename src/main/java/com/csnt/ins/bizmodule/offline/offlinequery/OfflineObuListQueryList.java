 ...
 */
public class OfflineObuListQueryList implements IReceiveService {

    private Logger logger = LoggerFactory.getLogger(QueryVehicleService.class);

    private final String serviceName = "[1011Obu列表查询]";
    ...
    public Result entry(Map dataMap) {
        ...
        try {
            ...
            //obu号
            String obuId = record.get("obuId") == null ? null : record.get("obuId").toString();
            //开始时间
            ...
            DbUtil.initParam("eo.registeredChannelId", channelId, condition, param, DbUtil.OPERATE.EQUAL);
            DbUtil.initParam("eo.operatorId", operatorId, condition, param, DbUtil.OPERATE.EQUAL);
            if (plateNum != null && plateColor != null) {
                DbUtil.initParam("eo.vehicleId", plateNum + "_" + plateColor, condition, param, DbUtil.OPERATE.EQUAL);
            } else if (plateNum == null && plateColor != null) {
                DbUtil.initParam("eo.vehicleId", "_" + plateColor, condition, param, DbUtil.OPERATE.RIGHT_LIKE);
            } else if (plateNum != null) {
                DbUtil.initParam("eo.vehicleId", plateNum + "_", condition, param, DbUtil.OPERATE.LEFT_LIKE);
            }
            DbUtil.initParam("eo.id", obuId, condition, param, DbUtil.OPERATE.EQUAL);
            DbUtil.initParam("eo.registeredTime", DateUtil.parseDate(startTime, DateUtil.FORMAT_YYYY_MM_DDTH_HMMSS), condition, param, DbUtil.OPERATE.GREATER_THAN_EQUAL);
            DbUtil.initParam("eo.registeredTime", DateUtil.parseDate(endTime, DateUtil.FORMAT_YYYY_MM_DDTH_HMMSS), condition, param, DbUtil.OPERATE.LESS_THAN_EQUAL);
            ...
            //查询数据库数据
            SqlPara sqlPara = DbUtil.getSqlPara("queryOfflineObuList", kv);
            //查询总条数
            SqlPara sqlSizePara = DbUtil.getSqlPara("queryOfflineObuSize", sizeKv);
